...
public class MemberCardAction extends BaseAction {
	...
	private JFreeChart formChart(){
		...
		String[][] cardStateDis = saleperformanceManage.getMemberCardDistribution(DayTransformer.transformToMonth(date));
		for(int i=0;i<cardStateDis.length;i++){
			data.setValue(cardStateDis[i][0], Integer.valueOf(cardStateDis[i][1]));
		}
		JFreeChart pieChart = ChartFactory.createPieChart("会员卡状态分布图", data, true, true, false);
