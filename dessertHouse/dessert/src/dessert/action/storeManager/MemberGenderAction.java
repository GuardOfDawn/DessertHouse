...
@Controller
public class MemberGenderAction extends BaseAction{
	...
	private JFreeChart formPipeChart(){
		...
		String[][] genderDis = saleperformanceManage.getMemberGenderDistribution(DayTransformer.transformToMonth(date));
		for(int i=0;i<genderDis.length;i++){
			data.setValue(genderDis[i][0], Integer.valueOf(genderDis[i][1]));
		}
		JFreeChart pieChart = ChartFactory.createPieChart("会员性别分布图", data, true, true, false);
