...
@Controller
public class MemberResidualAction extends BaseAction {
	...
	private JFreeChart formChart(){
		...
		String[][] residualDis = saleperformanceManage.getMemberResidualDis(DayTransformer.transformToMonth(date));
		for(int i=0;i<residualDis.length;i++){
			data.setValue(residualDis[i][0], Integer.valueOf(residualDis[i][1]));
		}
		JFreeChart pieChart = ChartFactory.createPieChart("会员余额状况分布图", data, true, true, false);
