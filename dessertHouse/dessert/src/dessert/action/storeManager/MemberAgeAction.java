...
@Controller
public class MemberAgeAction extends BaseAction{
	...
	private JFreeChart formChart(){
		...
		String[][] ageDis = saleperformanceManage.getMemberAgeDistribution(DayTransformer.transformToMonth(date));
		for(int i=0;i<ageDis.length;i++){
			data.setValue(ageDis[i][0], Integer.valueOf(ageDis[i][1]));
		}
		JFreeChart pieChart = ChartFactory.createPieChart("会员年龄分布图", data, true, true, false);
