...
@Controller
public class StoreOrderingAction extends BaseAction{
	...
	private JFreeChart formChart(){
		...
		String[][] storeOrderingDis = saleperformanceManage.getStoreOrderingDis(DayTransformer.transformToMonth(date));
		for(int i=0;i<storeOrderingDis.length;i++){
			data.setValue(Double.parseDouble(storeOrderingDis[i][1]), storeOrderingDis[i][0], storeOrderingDis[i][0]);
		}
		JFreeChart barChart = ChartFactory.createBarChart("店铺预订分布图", "店铺名称", 
                "预订额", data, PlotOrientation.VERTICAL, true, true, false);
